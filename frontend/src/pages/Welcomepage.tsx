...
const API_BASE_URL = process.env.REACT_APP_API_URL ||'https://ddcf6bc6761a.ngrok-free.app';
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <div className="mb-4">
        <Link
          to="/templates"
          className="inline-block px-4 py-2 bg-white text-black border-2 border-pink-300 rounded-lg font-semibold hover:bg-pink-100 hover:border-pink-400 transition-colors duration-200"
        >
          ← Back
        </Link>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 bg-white rounded-lg shadow-md p-6 flex flex-col">
              ...
              <div key={index} className="mb-6 p-4 border border-gray-200 rounded-lg">
                <div className="flex gap-4 mb-4 items-center">
                  ...
                  <div className="flex border border-gray-300 rounded-md overflow-hidden">
                    <button 
                      className={`px-4 py-2 text-sm ${workflow.type === 'payload' ? 'bg-pink-400 text-white' : 'bg-white text-gray-700'}`}
                      onClick={() => handleWorkflowTypeChange(index, 'payload')}
                    >
                      Payload
                    </button>
                    <button 
                      className={`px-4 py-2 text-sm ${workflow.type === 'weburl' ? 'bg-pink-400 text-white' : 'bg-white text-gray-700'}`}
          ...
          <button
            ...
            disabled={isLoading}
            className={`${isLoading ? 'bg-pink-400' : 'bg-pink-400 hover:bg-pink-500'} text-white py-2 rounded-md mt-auto flex justify-center items-center`}
          >
